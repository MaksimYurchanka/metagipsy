...
const AnalysisPage: React.FC = () => {
  console.log('🚀 ANALYSIS PAGE v9: Starting render (stable version)...');
  
  // ✅ STABLE: Direct store access without complex subscriptions
  ...
  console.log('✅ ANALYSIS PAGE v9: Store subscriptions successful, messages:', messages.length);

  // ✅ ULTRA STABLE: Simple function without useCallback to prevent dependency issues
  const handleAnalyze = async (analysisRequest: any) => {
    console.log('🚀 ANALYSIS PAGE v9: Received analysisRequest:', analysisRequest);
    ...
    console.log('🚀 ANALYSIS PAGE v9: Extracted data:', {
    ...
    if (!newMessages || newMessages.length === 0) {
      console.error('❌ ANALYSIS PAGE v9: No messages found');
    ...
    try {
      ...
      if (options.useClaudeAnalysis) {
        console.log('🤖 ANALYSIS PAGE v9: Using Claude API');
        await performClaudeAnalysis(analysisRequest, store);
      } else {
        console.log('⚡ ANALYSIS PAGE v9: Using local analysis');
        await performLocalAnalysis(newMessages, store);
      }

    } catch (error) {
      console.error('❌ ANALYSIS PAGE v9: Analysis failed:', error);
      const store = useAnalysisStore.getState();
      ...
      console.log('✅ ANALYSIS PAGE v9: Analysis completed');
    }
  };

  console.log('🚀 ANALYSIS PAGE v9: About to render, messages:', messages.length);
...
async function performClaudeAnalysis(analysisRequest: any, store: any) {
  ...
  try {
    ...
    if (response?.scores) {
      store.setScores(response.scores);
      
      if (response.summary) {
        store.setSessionSummary(response.summary);
      }
