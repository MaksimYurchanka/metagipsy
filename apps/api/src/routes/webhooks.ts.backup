  ...
  async 'payment_intent.succeeded'(event: Stripe.Event) {
    ...
    if (planType !== 'unknown') {
      ...
      await updateUserSubscription(customerId, {
        ...
        subscriptionEnd,
        dailyLimit,
        dailyUsage: 0, // Reset usage on new subscription
        usageResetDate: new Date(Date.now() + 24 * 60 * 60 * 1000) // Tomorrow
      });
...
router.post('/stripe', express.raw({ type: 'application/json' }), async (req: Request, res: Response) => {
  ...
  if (!signature) {
    logger.error('Missing Stripe signature', { requestId, ip: req.ip });
